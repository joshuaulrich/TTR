...
function(HL, accel=c(.02,.2)) {
  ...
  for (i in 2:NROW(sar)) {
    ...
    if (sig[i-1] == 1)  sig[i] <- ifelse(HL[i,2]>sar[i-1], 1,-1)  else
    if (sig[i-1] ==-1)  sig[i] <- ifelse(HL[i,1]<sar[i-1],-1, 1)

    # Create EP (Extreme Price) vector
    if (sig[i-1] == 1)  ep[i] <- ifelse(HL[i,1]>ep[i-1], HL[i,1], ep[i-1])  else
    if (sig[i-1] ==-1)  ep[i] <- ifelse(HL[i,2]<ep[i-1], HL[i,2], ep[i-1])

    # Create AF (Acceleration Factor) vector
    if (sig[i] == sig[i-1]) {
      if(sig[i] == 1)  af[i] <- ifelse(ep[i]>ep[i-1], ifelse(af[i-1]==accel[2], af[i-1], accel[1]+af[i-1]), af[i-1])  else
      if(sig[i] ==-1)  af[i] <- ifelse(ep[i]<ep[i-1], ifelse(af[i-1]==accel[2], af[i-1], accel[1]+af[i-1]), af[i-1])
    } else
      af[i] <- accel[1]
    ...
    if (sig[i] == sig[i-1]) {
      ...
      if(sig[i] == 1)  sar[i] <- ifelse(sar[i] < min(HL[(i-1):i,2]), sar[i], min(HL[(i-1):i,2]))  else
      if(sig[i] ==-1)  sar[i] <- ifelse(sar[i] > max(HL[(i-1):i,1]), sar[i], max(HL[(i-1):i,1]))
    } else
      sar[i] <- ep[i-1]
  }
